package com.example.socialmediaapplicaition.utils

object Constants {

    const val TAG = "MyApplication"
    const val PREFS_TOKEN_FILE = "PREFS_TOKEN_FILE"
    const val USER_ID = "USER_ID"
    const val USER_PROFILE = "USER_PROFILE"
    const val USER_NAME = "USER_NAME"
    const val USER_STATUS = "USER_STATUS"

    const val CALLING_ACTIONS = "CALLING_ACTIONS"
    const val ADD_USER_ACTION = "ADD_USER_ACTION"

    const val MESSAGE_TYPE_TEXT = "TEXT"
    const val MESSAGE_TYPE_IMAGE = "IMAGE"
    const val MESSAGE_TYPE_VIDEO = "VIDEO"

    const val ONLINE = "Online"

    const val SAVE = "SAVE"
    const val REMOVE = "REMOVE"

    // User data class
    const val NAME = "name"
    const val ONLINE_STATUS = "onlineStatus"
    const val PROFILE = "profile"
    const val STATUS = "status"


    //For Chat reply
    const val ACTION_THUMBS_UP = "thumbsUp"
    const val ACTION_THUMBS_DOWN = "thumbsDown"
    const val ACTION_LAUGHING = "laughing"
    const val ACTION_SURPRISED = "surprised"
    const val ACTION_SED = "sed"
    const val ACTION_REPLY = "REPLY"
    const val ACTION_DELETE = "DELETE"


}
