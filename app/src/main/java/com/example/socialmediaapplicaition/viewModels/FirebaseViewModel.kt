...
class FirebaseViewModel @Inject constructor(private val repository: FirebaseRepository) :
    ...
    private val _getAllChatMessages =
        MutableStateFlow<NetworkResult<ArrayList<ChatMessageModel>>?>(NetworkResult.Loading())
    val getAllChatChatMessages: StateFlow<NetworkResult<ArrayList<ChatMessageModel>>?> =
        _getAllChatMessages
    ...
    private val _userSavedPost =
        MutableStateFlow<NetworkResult<ArrayList<Post>>?>(NetworkResult.Loading())
    val userSavedPost: MutableStateFlow<NetworkResult<ArrayList<Post>>?> = _userSavedPost

    private val _updateUserOnlineStatus =
    ...
    fun addChatMessageToDatabase(chatMessage: ChatMessageModel, chatRoomId: String) =
    ...
    fun searchUsers(keyPoint: String, yourUserId: String) {
        allUsers.value?.data?.let { users ->
            val filteredUsers = users.filter { user ->
                user.id != yourUserId && user.name.contains(keyPoint, ignoreCase = true)
            }
    ...
    fun searchContacts(keyPoint: String, yourUserId: String) {
        getAllChatHistory.value?.data.let { users ->
            val filteredUsers = users?.filter { user ->
                val otherUser =
                    if (user.userList[0].id != yourUserId) user.userList[0] else user.userList.getOrNull(
                        1
                    )
        ...
        try {
            repository.getAllPreviousChat().collect { result ->
                ...
                when (result) {
                    is NetworkResult.Success -> {
                        val filteredChats =
                            result.data?.filter { chat -> chat.chatroomId.contains(userId) }
                        Log.d("checkingChatList", filteredChats.toString())
                        if (filteredChats != null) {
                            _getAllChatHistory.value =
                                NetworkResult.Success(filteredChats.toMutableList())
                        } else {
                            _getAllChatHistory.value =
                                NetworkResult.Success(ArrayList()) // No matches found
                        }
                    }

                    is NetworkResult.Error -> {
                        _getAllChatHistory.value =
                            NetworkResult.Error(result.toString()) // Pass along the error result
                    }
    ...
    fun deleteChatMessage(chat: ChatMessageModel, userId: String, chatRoomId: String) =
        viewModelScope.launch {
    ...
    fun updateChatReaction(chat: ChatMessageModel, user: Reactions, chatRoomId: String) =
        viewModelScope.launch {
            _deleteChatMessage.value = NetworkResult.Loading()
