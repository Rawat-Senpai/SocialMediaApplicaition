...
class FirebaseRepositoryImpl @Inject constructor(private val firebaseFirestore: FirebaseFirestore) :
    ...
    override suspend fun getAllFilteredPost(): NetworkResult<Posts> {
        return try {
            ...
            if (snapshot != null && !snapshot.isEmpty) {
                ...
                for (document in snapshot.documents) {
                    val user = document.toObject(Post::class.java)
                    user?.let {
                        userList.add(it)
                    }
                }
                Log.d("checkingResponse1Size1", userList.size.toString())
                NetworkResult.Success(userList)
            } else {
                NetworkResult.Error("No users found")
            }
